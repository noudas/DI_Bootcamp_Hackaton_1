...
class Saving_Goals:
    ...
    @classmethod
    def update_goal(cls, db: DB_Connect, goal_id, new_goal_name=None, new_target_amount=None, new_current_amount=None, new_due_date=None):
        """
        ...
        query = """
            ...
            SET goal_name = %s, target_amount = %s, current_savings = %s, due_date = COALESCE(%s, due_date), updated_at = CURRENT_TIMESTAMP 
            WHERE goal_id = %s
        """
        values = (
            new_goal_name or cls(db).goal_name,
            new_target_amount or cls(db).target_amount,
            new_current_amount or cls(db).current_amount,
    ...
    def delete_goal(cls, db: DB_Connect, goal_id):
        ...
        """
        query = "DELETE FROM savings_goals WHERE goal_id = %s"
        params = (goal_id,)
